...
function calculateInput(){
    ...
    if($('#form-input').length > 0 && $("#form-input")[0].checkValidity()){
        ...
        document.getElementById("quotaValue").innerHTML = format("###.##0,#0", quotaValue);
        var totalToPayValue = totalToPay(quotaValue, numberOfQuotesValue, 2);
        document.getElementById("totalToPayValue").innerHTML = format("###.##0,#0", totalToPayValue);
        var interestValue = interest(totalToPayValue, capitalValue, 2);
        document.getElementById("interestValue").innerHTML = format("###.##0,#0", interestValue);
    }    
}
